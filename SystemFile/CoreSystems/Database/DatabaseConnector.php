...
class DatabaseConnector
{
                    ...
                    if(!empty($mode)) {
                        if ($mode == "Separate" and !empty($value) and is_array($value)) {
                            ...
                            foreach ($value as $valu) {
                                array_push($this->whereTerms, $valu);
                            }
                        ...
                        if(!$this->isSubQuery){
                            $col = rtrim($col, ',');
                            $bq = "";
                            foreach($this->whereTerms as $v){
                                $bq .= "?,";
                            }
                            $bq = rtrim($bq, ',');
                            $this->QueryCommand .= "INSERT INTO {$table} ({$col}) VALUES ({$bq})";
                        }else{
                            $col = rtrim($col, ',');
                            $bq = "";
                            foreach($this->whereTerms as $v){
                            ...
                            $this->subQueryCommand .= "INSERT INTO {$table} ({$col}) VALUES ({$bq})";
                        }
                        return $this;
                    }else{
                    ...
                    if (!empty($terms1) and is_string($terms1) and !empty($terms2) and is_string($terms2) and !empty($type) and is_string($type)) {
                        if ($type == "eq" or $type == "noteq" or $type == "subquery") {
                            ...
                            if (!$this->isSubQuery) {
                                ...
                                $this->now = "WHERE";
                                array_push($this->whereTerms, htmlspecialchars($terms2));
                            } else {
    ...
    public function CountRow(){
        ...
        if($this->executed){
            ...
            $CreateException->EchoError($this->SysData, $this->langPack, "MustBeRun", dirname(__FILE__)."/".$this->LogDir);
            exit;
        }
    }
